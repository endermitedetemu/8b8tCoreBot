...
class DiscordBot {
    ...
    async start() {
        return new Promise((resolve) => {
            this.client.once("ready", async () => {
                console.log(
                    `Bot de Discord conectado como ${this.client.user.tag}`,
                );
                try {
                    this.channels.main = await this.client.channels.fetch(
                        config.discord.channels.main,
                    );
                    this.channels.cmd = await this.client.channels.fetch(
                        config.discord.channels.cmd,
                    );
                    ...
                    console.log(
                        `Canal comandos cargado: ${this.channels.cmd.name}`,
                    );
                    console.log(
    ...
    async registerSlashCommands() {
        const commands = [
            ...
            new SlashCommandBuilder()
                .setName("idlist")
                .setDescription("Ver usuarios registrados"),
            new SlashCommandBuilder()
                .setName("players")
            ...
            new SlashCommandBuilder()
                ...
                .addStringOption((o) =>
                    o
                        .setName("jugador")
                        .setDescription("Jugador objetivo")
                        .setRequired(true),
                )
                .addStringOption((o) =>
                    o
                        .setName("mensaje")
                        .setDescription("Mensaje a spamear")
                        .setRequired(true),
                ),
            new SlashCommandBuilder()
                .setName("ddsoff")
            ...
            new SlashCommandBuilder()
                .setName("addid")
                .setDescription(
                    "Vincular tu Discord con tu usuario de Minecraft",
                )
                .addStringOption((o) =>
                    o
                        .setName("usuario")
                        .setDescription("Tu usuario de Minecraft")
                        .setRequired(true),
                ),
            new SlashCommandBuilder()
                .setName("w")
                .setDescription(
                    "Enviar mensaje privado a un usuario registrado",
                )
                .addStringOption((o) =>
                    o
                        .setName("usuario")
                        .setDescription("Usuario MC, ID Discord o all")
                        .setRequired(true),
                )
                .addStringOption((o) =>
                    o
            ...
            new SlashCommandBuilder()
                ...
                .addStringOption((o) =>
                    o
                        .setName("usuario")
                        .setDescription("Usuario de Minecraft (opcional)"),
                ),
            new SlashCommandBuilder()
                .setName("info")
            ...
            new SlashCommandBuilder()
                .setName("8b8tlogs")
                .setDescription(
                    "Genera y envía los PDFs de join dates y métricas del servidor",
                ),
            new SlashCommandBuilder()
                .setName("8b8tplayer")
            ...
            new SlashCommandBuilder()
                .setName("dupetoggle")
                .setDescription(
                    "Activar o desactivar el frame dupe (solo owner)",
                )
                .addStringOption((o) =>
                    o
                        .setName("estado")
        ...
        try {
            ...
            console.log(
                `${commands.length} comandos slash registrados correctamente`,
            );
        } catch (error) {
    ...
    setMessagingHandler(messagingHandler) {
        ...
        this.client.on("interactionCreate", async (interaction) => {
            ...
            if (commandName === "restart") {
                ...
                await interaction.reply({
                    content: "Reiniciando el bot completo...",
                    ephemeral: true,
                });
            ...
            if (commandName === "banstatus")
                return handleBanStatus(interaction, this.banManager);
            if (commandName === "banlist")
                return handleBanList(interaction, this.banManager);
                ...
                if (resultado) {
                    return interaction.reply({
                        content:
                            ...
                            targetUser +
                            "** añadido. Total: " +
                            this.banManager.getList().length,
                        ephemeral: true,
                    });
                } else {
                    return interaction.reply({
            ...
            if (commandName === "unbanall") {
                ...
                if (cantidad === 0)
                    return interaction.reply({
                        content: "No hay usuarios baneados actualmente.",
                        ephemeral: true,
                    });
                return interaction.reply({
                    content: "Se removieron **" + cantidad + "** usuarios.",
                    ephemeral: true,
                });
            }
            ...
            if (commandName === "ddsstatus") {
                ...
                const fields = targets.map((target) => {
                    ...
                    const timeSinceLastSeen = Math.floor(
                        (Date.now() - info.lastSeen) / 1000,
                    );
                    const estado = info.ignored
                        ? "🔇 Ignorado (sin respuesta +10s)"
                        : this.mcBot &&
                            this.mcBot.players &&
                            this.mcBot.players[target]
            ...
            if (commandName === "addid") {
                ...
                return interaction.reply({
                    content: "Vinculado con **" + minecraftUsername + "**",
                    ephemeral: true,
                });
            }

            if (commandName === "idlist") {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription("Total: " + userIds.length)
                    .addFields({
                        name: "Vinculaciones",
                        value: userList.join("\n"),
                        inline: false,
            ...
            if (commandName === "w") {
                ...
                const result =
                    await this.messagingHandler.sendMessageFromDiscord(
                        this.mcBot,
                        this.client,
                        interaction.user.id,
                        target,
            ...
            if (!this.mcBot || !this.mcBot.player) {
                return interaction.reply({
                    content: "El bot de Minecraft no esta conectado.",
                    ephemeral: true,
                });
            }
            ...
            if (commandName === "players") {
                try {
                    await interaction.deferReply({ ephemeral: true });
                    const embed = await handleCommand(
                        "!players",
                        this.mcBot,
                        interaction.user.username,
                        this.banManager,
                    ...
                    try {
                        return interaction.editReply({
                            content: "Error al obtener jugadores.",
                        });
                    } catch (e) {}
                }
            }
            ...
            if (commandName === "ping") {
                ...
                return interaction.reply({
                    content: result || "Jugador no encontrado.",
                    ephemeral: true,
                });
            }
            ...
            if (commandName === "tpa") {
                ...
                return interaction.reply({
                    content: result || "Comando ejecutado.",
                    ephemeral: true,
                });
            }
            ...
            if (commandName === "8b8tlogs") {
                try {
                    ...
                    const files = [];
                    const fs = require("fs");
                    if (fs.existsSync(paths.players))
                        files.push(
                            new AttachmentBuilder(paths.players, {
                                name: "8b8t_players.pdf",
                            }),
                        );
                    if (fs.existsSync(paths.metrics))
                        files.push(
                            new AttachmentBuilder(paths.metrics, {
                                name: "8b8t_metrics.pdf",
                            }),
                        );
                    if (paths.graphs && fs.existsSync(paths.graphs))
                        files.push(
                            new AttachmentBuilder(paths.graphs, {
                                name: "8b8t_graphs.pdf",
                            }),
                        );
                    if (files.length === 0)
                        return interaction.editReply({
                            content: "No se pudieron generar los PDFs.",
                        });
                    return interaction.editReply({
                        content: `PDFs generados. Jugadores tracked: ${stats.total} | Pico online: ${stats.peak}`,
                        files,
                    });
                } catch (err) {
                    ...
                    try {
                        return interaction.editReply({
                            content: "Error al generar PDFs: " + err.message,
                        });
                    } catch (e) {}
                }
            }
